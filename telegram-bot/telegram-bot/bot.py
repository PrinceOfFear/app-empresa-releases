...
async def renovar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Guardar status atual ANTES de modificar
    was_cancelled = lic.get("status") == "cancelled"
    
    # REGRA IMPORTANTE:
    # - Licença CANCELADA: SEMPRE começa do ZERO (hoje)
    # - Licença ATIVA com dias restantes: adiciona a partir da expiração
    # - Licença EXPIRADA (não cancelada): começa de hoje
    
    if was_cancelled:
        # Licença foi CANCELADA: começa do zero absoluto
        base_date = date.today()
        # Gerar nova chave obrigatoriamente
        lic["key"] = generate_license_key()
        logger.info(f"Renovando licenca CANCELADA - comecando do zero: {base_date}")
    else:
        # Licença ativa ou apenas expirada
        try:
            current_expires = date.fromisoformat(lic.get("expires", ""))
            # Se ainda tem dias restantes, adiciona a partir da expiração
            ...
            if current_expires > date.today():
                ...
                base_date = date.today()
                logger.info(f"Renovando licenca EXPIRADA - comecando de hoje: {base_date}")
        except ValueError:
            base_date = date.today()
